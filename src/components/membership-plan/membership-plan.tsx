...
const MembershipPlan = ({ subscription }: MembershipPlanProps) => {
  ...
  const openPortal = async () => {
    ...
    const response = await fetch("/api/subscription/manage", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(paylaod) });
        ...
        <div className="flex flex-col justify-between pt-4 pb-4 md:flex-row md:pb-0">
          <div>
            <div className="flex items-center gap-2">
              <span className="py-1 px-3 uppercase rounded bg-white/20">{subscription.default_payment_method ? subscription.default_payment_method.card.brand : subscription.customer.invoice_settings.default_payment_method.card.brand}</span>
